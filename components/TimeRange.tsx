...
const TimeRange = () => {
  ...
  const previousDay = () => {
    dispatch(userPreferencesServices.actions.updateTimeRange({
      from: moment(timeRange.from * 1000).subtract(1, 'day').unix(),
      to: moment(timeRange.to * 1000).subtract(1, 'day').unix(),
    }))
  }

  const nextDay = () => {
    dispatch(userPreferencesServices.actions.updateTimeRange({
      from: moment(timeRange.from * 1000).add(1, 'day').unix(),
      to: moment(timeRange.to * 1000).add(1, 'day').unix(),
    }))
  }
